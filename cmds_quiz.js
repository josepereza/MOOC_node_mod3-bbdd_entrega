...
// Delete quiz (identified by <id>) in the DB
...
exports.play = async (rl) =>  {
  ...
  let quizzes = shuffle(await Quiz.findAll());
  ...
  for (let quiz of quizzes) {
    let answerUser = await rl.questionP(quiz.question);

    if (answerUser.toLowerCase().trim() === quiz.answer.toLowerCase().trim()) {
      rl.log(`The answer "${answerUser}" is right!`);
      points++;
    } else {
      rl.log(`The answer "${answerUser}" is wrong!`);
      break;
    }
  }
  ...
  let name = await rl.questionP("Enter user name ...");
  let userDB = await User.findOne({ where: { name } });
  if (!userDB) {
    userDB = await User.create({ name, age: 16 });
  }
  
  await Score.create({ wins: points, userId: userDB.id });
}
